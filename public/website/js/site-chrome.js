...
(function () {
  ...
  // ── PostHog Web Analytics (browser SDK) ───────────────────────────────────
  // Loads the official posthog-js bundle and initialises it ONLY when a key is
  // configured server-side (written into KADR_SITE_CONFIG.posthog by the site
  // generator). When analytics are off, nothing loads — no network calls, no
  // cookies. Enables automatic pageviews, pageleave and autocapture, which power
  // PostHog's Web Analytics dashboard (traffic, sessions, sources, web vitals).
  function initPosthog () {
    try {
      var ph = window.KADR_SITE_CONFIG && window.KADR_SITE_CONFIG.posthog
      if (!ph || !ph.enabled || !ph.key) return
      if (window.posthog && window.posthog.__loaded) return

      var apiHost = ph.apiHost || 'https://us.i.posthog.com'
      var assetsHost = ph.assetsHost || apiHost

      // Official posthog-js loader snippet (stub queues calls until the async
      // bundle from assetsHost finishes loading).
      ...
      // We use the standard PostHog asset origin derivation inside the snippet
      // (api_host -> -assets), but pass the configured hosts explicitly so
      // EU/self-hosted work too.
      window.posthog.init(ph.key, {
        api_host: apiHost,
        ui_host: apiHost,
        ...
        autocapture: true,
        // Respect Do-Not-Track and keep cookies first-party.
        respect_dnt: true,
        persistence: 'localStorage+cookie',
        loaded: function () { /* ready */ }
      })

      // If the SDK bundle lives on a non-standard assets host (EU/self-hosted),
      // make sure the loader points there rather than the derived default.
      if (assetsHost && assetsHost.indexOf('-assets.i.posthog.com') === -1) {
        // Self-hosted: the snippet's derivation is a no-op, bundle loads from api_host origin.
      }
    } catch (e) {
